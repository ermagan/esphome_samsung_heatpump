...
namespace esphome
{
  ...
  {
    static const char *TAG = "samsung_hp";

    climate::ClimateTraits Samsung_HP_Climate::traits()
    {
    ...
    void Samsung_HP_Climate::control(const climate::ClimateCall &call)
    {
      traits();
    ...
    void Samsung_HP_Device::write_target_temperature(float value)
    {
      auto data = protocol->get_target_temp_message(address, value);
      samsung_hp->send_bus_message(data);
    }

    void Samsung_HP_Device::write_mode(Mode value)
    {
      auto data = protocol->get_mode_message(address, value);
      samsung_hp->send_bus_message(data);
    }

    void Samsung_HP_Device::write_fanmode(FanMode value)
    {
      auto data = protocol->get_fanmode_message(address, value);
      samsung_hp->send_bus_message(data);
    }

    void Samsung_HP_Device::write_power(bool value)
    {
