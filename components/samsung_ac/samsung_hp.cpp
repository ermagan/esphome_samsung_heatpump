...
namespace esphome
{
  namespace samsung_hp
  {
    ...
    void Samsung_HP::setup()
    {
    }

    void Samsung_HP::update()
    {
      ESP_LOGW(TAG, "update");
    ...
    void Samsung_HP::send_bus_message(std::vector<uint8_t> &data)
    {
      out_.insert(out_.end(), data.begin(), data.end());
    }

    void Samsung_HP::dump_config()
    {
      ESP_LOGCONFIG(TAG, "Samsung_HP:");
    ...
    float Samsung_HP::get_setup_priority() const { return setup_priority::DATA; }
  } // namespace samsung_hp
} // namespace esphome
