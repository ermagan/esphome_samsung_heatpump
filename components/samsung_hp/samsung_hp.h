...
namespace esphome
{
  namespace samsung_hp
  {
    ...
    class Samsung_HP : public PollingComponent,
    ...
    public:
      ...
      void /*MessageTarget::*/ register_device(Samsung_HP_Device *device)
      {
      ...
      {
        Samsung_HP_Device *dev = find_device(address);
        if (dev != nullptr)
    ...
    protected:
      Samsung_HP_Device *find_device(const std::string address)
      {
        for (int i = 0; i < devices_.size(); i++)
        {
          Samsung_HP_Device *dev = devices_[i];
      ...
      std::vector<Samsung_HP_Device *> devices_;
